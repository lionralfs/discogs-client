...
var tests = (module.exports = [
    ...
    },
    {
        ...
        test: function () {
            nock('https://www.example.com').get('/labels/1').reply(200, '{"result": "example.com is host"}');

            nock('https://api.discogs.com').get('/labels/1').reply(200, '{"result": "api.discogs.com is host"}');

            var clientWithOptions = new DiscogsClient(null, null, { host: 'www.example.com' });
            var clientWithoutOptions = new DiscogsClient();

            clientWithOptions._request(
                { url: '/labels/1' },
                wru.async(function (err, data) {
                    wru.assert('No error', !err);
