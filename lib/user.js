...
module.exports = function (client) {
    ...
    user.profile = deprecate.function(user.getProfile, 'user.profile: Use user.getProfile instead');
    ...
    user.getInventory = function (user, params, callback) {
        ...
        if (arguments.length === 2 && typeof params === 'function') {
            ...
            // Add paging params when present
            path = util.addParams(path, params);
        }
        client.get(path, callback);
    };

    user.inventory = deprecate.function(user.getInventory, 'user.inventory: Use user.getInventory instead');
    ...
    user.identity = deprecate.function(user.getIdentity, 'user.identity: Use user.getIdentity instead');
    ...
    user.contributions = deprecate.function(
    ...
    user.submissions = deprecate.function(user.getSubmissions, 'user.submissions: Use user.getSubmissions instead');
