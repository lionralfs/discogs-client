...
import { version as _version, homepage } from '../package.json';
...
var defaultConfig = {
    ...
    port: 443,
    userAgent: 'DisConnectClient/' + _version + ' +' + homepage,
    apiVersion: 'v2',
...
var queue = require('./queue.js').default({
...
export class DiscogsClient {
    ...
    _rawRequest(options, callback) {
        ...
        var req = request(requestOptions, function (res) {
            var data = '',
                rateLimit = null,
                add = function (chunk) {
                    data += chunk.toString();
            ...
            switch (res.headers['content-encoding']) {
                case 'gzip':
                    var gunzip = createGunzip().on('data', add).on('end', passData);
                    res.pipe(gunzip);
                    break;
                case 'deflate':
                    var inflate = createInflate().on('data', add).on('end', passData);
                    res.pipe(inflate);
                    break;
                default:
     ...
     */
    oauth() {
        var OAuth = require('./oauth.js').default;
     ...
     */
    database() {
        return require('./database.js').default(this);
    }
    /**
     ...
     */
    marketplace() {
        return require('./marketplace.js').default(this);
    }
    /**
     ...
     */
    user() {
        return require('./user.js').default(this);
    }
}
