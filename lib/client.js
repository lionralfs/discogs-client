...
function DiscogsClient(userAgent, oauth, options) {
    ...
    this.oauth =
        typeof oauth === 'object' ? merge({}, oauth) : { version: '1.0', signatureMethod: 'HMAC-SHA1', status: null };
...
DiscogsClient.prototype.getRequestToken = function (consumerKey, consumerSecret, callbackUrl, callback) {
    var oauth = this.oauth,
        oa = new OAuth({ consumer: { public: consumerKey, secret: consumerSecret } });
    oauth.consumerKey = consumerKey;
    oauth.consumerSecret = consumerSecret;
    oauth.status = 'request';
    this._rawRequest(
        { url: this.config.oauthRequestUrl + '?oauth_callback=' + oa.percentEncode(callbackUrl) },
        function (err, data) {
            if (!err) {
                if (data) {
...
DiscogsClient.prototype.getAccessToken = function (verifier, callback) {
    var oauth = this.oauth,
        oa = new OAuth({ consumer: { public: oauth.consumerKey, secret: oauth.consumerSecret } });
    oauth.status = 'access';
    this._rawRequest(
        { url: this.config.oauthAccessUrl + '?oauth_verifier=' + oa.percentEncode(verifier) },
        function (err, data) {
            if (!err) {
...
DiscogsClient.prototype.about = function (callback) {
    ...
    this.get('', function (err, data) {
        if (data) {
            data.disconnect = {
                version: pkg.version,
                userAgent: self.userAgent || this.config.customHeaders['User-Agent'],
            };
        }
        callback(err, data);
    });
...
DiscogsClient.prototype._rawRequest = function (options, callback) {
    ...
    if (this.oauth.status) {
        var oa = new OAuth({ consumer: { public: this.oauth.consumerKey, secret: this.oauth.consumerSecret } }),
            fullUrl =
                urlParts.protocol && urlParts.host ? urlParts.href : 'https://' + this.config.host + urlParts.path,
            authObj = oa.authorize(
                { method: method, url: fullUrl },
                { public: this.oauth.token, secret: this.oauth.tokenSecret }
            );
