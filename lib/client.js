...
DiscogsClient.prototype.getRequestToken = function (consumerKey, consumerSecret, callbackUrl, callback) {
    ...
    this._rawRequest(
        { url: config.oauthRequestUrl + '?oauth_callback=' + oa.percentEncode(callbackUrl) },
        function (err, data) {
            if (!err) {
                if (data) {
...
DiscogsClient.prototype.getAccessToken = function (verifier, callback) {
    ...
    oauth.status = 'access';
    this._rawRequest(
        { url: (this.customConfig || defaultConfig).oauthAccessUrl + '?oauth_verifier=' + oa.percentEncode(verifier) },
        function (err, data) {
            if (!err) {
...
DiscogsClient.prototype.about = function (callback) {
    ...
    this.get('', function (err, data) {
        if (data) {
            data.disconnect = {
                version: pkg.version,
                userAgent: self.userAgent || (this.customConfig || defaultConfig).customHeaders['User-Agent'],
            };
        }
        callback(err, data);
    });
