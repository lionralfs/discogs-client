...
DiscogsClient.prototype.getAccessToken = function (verifier, callback) {
    ...
    oauth.status = 'access';
    this._rawRequest(
        { url: config.oauthAccessUrl + '?oauth_verifier=' + oa.percentEncode(verifier) },
        function (err, data) {
            if (!err) {
...
DiscogsClient.prototype.about = function (callback) {
    ...
    this.get('', function (err, data) {
        if (data) {
            data.disconnect = { version: pkg.version, userAgent: self.userAgent || config.customHeaders['User-Agent'] };
        }
        callback(err, data);
    });
};
