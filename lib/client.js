...
DiscogsClient.prototype.getRequestToken = function (consumerKey, consumerSecret, callbackUrl, callback) {
    var requestObject = merge({}, this.oauth),
    ...
    this._rawRequest(
        { url: config.oauthRequestUrl + '?oauth_callback=' + oa.percentEncode(callbackUrl), oauth: requestObject },
        function (err, data) {
            if (!err) {
...
DiscogsClient.prototype.about = function (callback) {
    this.get('', function (err, data) {
        if (data) {
            data.disconnect = { version: pkg.version, userAgent: this.userAgent || config.customHeaders['User-Agent'] };
        }
    ...
    if (oauth.status) {
        var oa = new OAuth({ consumer: { public: oauth.consumerKey, secret: oauth.consumerSecret } }),
            ...
            authObj = oa.authorize(
                ...
                { public: oauth.token, secret: oauth.tokenSecret }
            );
