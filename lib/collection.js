...
export default function (client) {
    ...
    collection.getFolders = function (user, callback) {
        return client.get('/users/' + escape(user) + '/collection/folders', callback);
    };
    ...
    collection.getFolder = function (user, folder, callback) {
        if (client.authenticated(2) || parseInt(folder, 10) === 0) {
            return client.get('/users/' + escape(user) + '/collection/folders/' + folder, callback);
    ...
    collection.getReleases = function (user, folder, params, callback) {
        if (client.authenticated(2) || parseInt(folder, 10) === 0) {
            var path = '/users/' + escape(user) + '/collection/folders/' + folder + '/releases';
    ...
    collection.getReleaseInstances = function (user, release, callback) {
        return client.get('/users/' + escape(user) + '/collection/releases/' + release, callback);
    };
    ...
    collection.addRelease = function (user, folder, release, callback) {
        ...
        return client.post(
            {
                url: '/users/' + escape(user) + '/collection/folders/' + (folder || 1) + '/releases/' + release,
                authLevel: 2,
            },
            null,
            callback
